...
class Chat(dspy.signatures):
    ...
    assisant_message: str = dspy.OutputField(desc="The response to the user message")
...
class Category(dspy.signatures):
    conversation_history: dspy.History = dspy.InputField(desc="The conversation history")
    user_message: str = dspy.InputField(desc="The user's message")
...
the user's preferences for a given category by adding relevant new informationan
and removing irrelevant information. Return the updated preferences and a brief 
explanation for the update decision."""
class UpdateCategory(dspy.signatures):
    conversation_history: dspy.History = dspy.InputField(desc="The conversation history")
    user_message: str = dspy.InputField(desc="The user's message")
    ...
    prefence_ids_to_remove: list[str] = dspy.OutputField(desc="Preference ids to remove for category, if any")
